...
// each mutant is initially sensitive and must mature (through a 1-step Poisson process) into a fully resistant phanotype upon replication
// each simulation is provided with an individual file of growth rates
// this code enables different alleles to have different mutation probabilities
...
float omega = 10 ; // transition rate from the intermediate to fully resistant phenotype
...
float *mutprobs ; // relative mutation probabilities
...
vector <QWORD> *ns,*nsr,*nr ; // nsr = intermediate between S and R
...
void init()
{
	...
	gr[0]=new float[strains*(1+2*alleles)] ;
	gr[1]=new float[strains*(1+2*alleles)] ;
	mutprobs=new float[alleles] ;
	ns=new vector<QWORD>[strains] ;
	nsr=new vector<QWORD>[strains] ;
	nr=new vector<QWORD>[strains] ;
	for (int i=0;i<strains*(1+2*alleles);i++) { fscanf(f,"%f",&gr[0][i]) ; }
	if (ferror(f) || feof(f)) err("error 1 when reading parameters") ;
	for (int i=0;i<strains*(1+2*alleles);i++) { fscanf(f,"%f",&gr[1][i]) ; }
	if (ferror(f) || feof(f)) err("error 2 when reading parameters") ;
	float totmutprob=0 ;
	for (int i=0;i<alleles;i++) { fscanf(f,"%f",&mutprobs[i]) ; totmutprob+=mutprobs[i] ; }
	for (int i=0;i<alleles;i++) mutprobs[i]/=totmutprob ; // normalize so that the probs sum up to one
	if (ferror(f) || feof(f)) err("error 3 when reading parameters") ;
	fclose(f) ;
}
...
QWORD run_once(int sam)
{
  int i,j,k,l;  
	ods.clear() ;
	finaln.clear() ;
	for (i=0;i<strains;i++) { ns[i].clear() ; nsr[i].clear() ; nr[i].clear() ; }
	tts.clear() ;
  const int L=strains*(2*alleles+1);
  tt=tt0=0 ;
	QWORD n[L] ;
  ...
  float od=0,od_add=0 ;
	...
	// for strains=2, alleles=2:
	// n[0] = strain 1_S, n[1] = strain 2_S,
	// n[2] = strain 1_I1, n[3] = strain 2_I1,
	// n[4] = strain 1_I2, n[5] = strain 2_I2,
	// n[6] = strain 1_R1, n[7] = strain 2_R1,
	// n[8] = strain 1_R2, n[9] = strain 2_R2,
  ...
  for (i=0;i<strains;i++) {
	...
	//cout <<"grr: " ;
	for (i=strains;i<strains*(2*alleles+1);i++) {
		if (sam==0) {
			cout<<gr[0][i]<<" " ; 
			cout<<gr[1][i]<<" " ; 
		}
	...
	if (sam==0) {
		cout <<endl<<" mu's= " ; 
		for (i=0;i<alleles;i++) cout<<mutprobs[i]<<" " ;
		cout <<endl ;
	}
  ...
  do {      // main loop 
    ...
    for (j=0;j<L;j++) {  // no death in this model
      ...
      QWORD db=binomial_2(b*dt,n[j]) ;
			if (j<strains || j>=strains*(1+alleles)) {
	      if (db>0) {         // replication
	        int dbmut ; 
	...
	//              cout<<j<<"->"<<c->gen<<","<<c->n<<" " ;
	          ...
	          // select an allele with prob mutprob[a]
	          int a=0 ;
	          double totprob=mutprobs[0], x=_drand48() ;
	          while (totprob<x) { a++ ; totprob+=mutprobs[a] ; }
						n[strains+j+strains*a]+=dbmut ;
	          //n[strains+j+strains*int(_drand48()*alleles)]+=dbmut ; // old version, same prob for all alleles
	          
						    //cout <<nc.gen<<":" ;
	//              cout <<"x"<<c->gen<<endl ;
	        }
	      }
	    } else {  // intermediate type transforms into a resistant one
	    	if (db>0) {
					QWORD dm=binomial_2(omega,db) ;
		    	n[j]+=db-dm ;
		    	n[j+strains*alleles]+=dm ;
		    }
	    }
    }
    ntot=0LL ;
    ...
    if (AB_type==RIF && tt>t_ant) {
    	if (c==1) { rif=RIF_rif0 ; dec=0 ; c=2 ; }
 			if (c>1) {
 				// od += rif + 3.3*dec;  // do not add since the model will be compared with corrected data, but the RIF contribution will be still outputted for plotting example trajectories
...
//    QWORD ntest=0 ;
		for (i=0;i<strains;i++) { 
			ns[i].push_back(n[i]) ; 
			QWORD nnsr=0,nnr=0 ;
			for (j=0;j<alleles;j++) nnsr+=n[strains+i+j*strains] ;
			for (j=0;j<alleles;j++) nnr+=n[strains+i+j*strains+strains*alleles] ;
			nsr[i].push_back(nnsr) ;
			nr[i].push_back(nnr) ; 
...
int main(int argc, char *argv[])
{
  ...
  int i,j,k;
	if (argc<14) { err(" Error:: arguments needed: NAME vol od_ini t_ant t_max t_regrowth od_regrowth nsamples mu_min mu_max dilf\nProgram terminated. \n"); } 
  else { 
    NAME=argv[1] ;
    vol=atof(argv[2]) ;
...
//    birthR=atof(argv[5]) ;
  ...
  if (argc<=15) { omega=atof(argv[14]) ; cout<<"omega="<<omega<<endl ; }
  _srand48(24) ;
  ...
  init() ;
    ...
    if (n>n_to_OD*vol*od_crit*0.5) { 
  		...
  		data<<mu_total<<" "<<tts[i]<<" "<<tts[j]<<" "<<tts[k] ;
			for (j=0;j<strains;j++) data<<" "<<ns[j][ns[j].size()-1]<<" "<<nsr[j][nsr[j].size()-1]<<" "<<nr[j][nr[j].size()-1] ;
			data<<endl;
  		...
  		if (fabs(tts[i]-t_regrowth)<ttol || tmax<t_ant || t_regrowth==100) { // save the data in case of a good fit
				...
				for (i=0;i<tts.size();i++) {
					ndata<<tts[i]<<"\t" ; 
					for (j=0;j<strains;j++) ndata<<ns[j][i]<<"\t"<<nsr[j][i]<<"\t"<<nr[j][i]<<"\t" ;
					ndata<<ods[i]<<endl ;
				}
			}
